...
def build_cumulative_load_chart(trainings: list[dict], planned_full_week: list[dict], monday_date: date) -> alt.Chart:
    ...
    done_line = base.mark_line(point=True, color="#ef4444").encode(
        y=alt.Y("Done:Q", title="Cumulative Foster Load"),
        tooltip=["Day", "Done"],
    ).transform_filter(alt.datum.Done != None)

    planned_line = base.mark_line(point=True, color="#60a5fa", strokeDash=[4, 4]).encode(
        y=alt.Y("Planned:Q"),
        tooltip=["Day", "Planned"],
    )

    return (planned_line + done_line).properties(height=250).configure_view(strokeWidth=0)
        ...
        with col_detail:
            ...
            if not selected_id:
                ...
                if athlete:
                    ...
                    planned_full = fetch_planned_trainings(token, selected_id, from_date, sunday)
                    planned_future = [t for t in planned_full if t["date_start"] > to_date]
                    ...
                    st.altair_chart(
                        build_cumulative_load_chart(trainings, planned_full, monday_date),
                        use_container_width=True,
                    )
